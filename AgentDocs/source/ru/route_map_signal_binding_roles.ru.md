# Signal Binding роли RouteMap

Документ описывает роли `SignalBindingRole`, которые связывают элементы RouteMap с `SignalId` и далее с `Modbus.DataMap` на экране `SignalId ↔ Modbus`.
Операторские модальные аварии, повторные подтверждения и обычные сообщения не являются `SignalBindingRole`:
их нужно настраивать в `Modbus.AlarmMap` через вкладку `Менеджер тревог`.
В этой таблице `Alarm area/Offset/Bit` задают входной бит показа диалога, `OK area/Offset/Bit`
задают отдельный acknowledgement-бит для кнопки `Хорошо`, `Repeat ms` управляет
повторным показом при активном alarm-бите, а `Pulse ms` задает длительность импульса
подтверждения. Роль `Fault` ниже только меняет визуальное состояние RouteMap-элемента и
не открывает модальный диалог.

Роль `State` и SignalId вида `*.state` являются legacy: старые JSON могут загрузиться, но актуальная схема RouteMap их удаляет из профиля, inventory и runtime-обработки. Базовое визуальное состояние задается статическим fallback в настройках объекта, а динамика приходит через `Fault`, `ActiveRoute`, `Visible`, `Text`, карточные `StartOffFeedback`/`StopOffFeedback` и командные роли.

## Карта и маршруты

| Роль | Направление | Тип | Назначение |
| --- | --- | --- | --- |
| `Visible` | `Read` | `Bool` | Управляет видимостью объекта на карте. |
| `Fault` | `Read` | `Bool` | Переводит объект в аварийное состояние при `true`. |
| `ActiveRoute` | `Read` | `Bool` | Показывает, что узел или линия входит в текущий активный маршрут. |
| `connection.status` | `Read` | `String` | Системный SignalId статуса общего Modbus Demo runtime, отображается в TopBar. |
| `connection.connected` | `Read` | `Bool` | Системный флаг доступности связи; при `false` команды блокируются, узлы и линии offline. |

## Узлы

| Роль | Направление | Тип | Назначение |
| --- | --- | --- | --- |
| `TargetCommand` | `ReadWrite` | `Bool` | Toggle-команда выбора узла как точки отправки. Клик пишет `true`; снятие выбора или переключение на другой узел пишет `false`. |
| `LoaderCommand` | `ReadWrite` | `Bool` | Toggle-команда выбора узла как точки возврата/загрузки. Клик пишет `true`; снятие выбора или переключение на другой узел пишет `false`. |

Если у узла `MenuKind = SendOnly`, используется `TargetCommand`. Если `MenuKind = SendAndReturn`, дополнительно используется `LoaderCommand`.

## Линии

| Роль | Направление | Тип | Назначение |
| --- | --- | --- | --- |
| `Visible` | `Read` | `Bool` | Показывает или скрывает линию. |
| `Fault` | `Read` | `Bool` | Помечает линию аварийной. |
| `ActiveRoute` | `Read` | `Bool` | Подсвечивает линию как часть активного маршрута. |

## Карточки

| Роль | Направление | Тип | Назначение |
| --- | --- | --- | --- |
| `Text` | `Read` | `UInt16` или `String` | Текстовый или кодовый статус карточки. Код `0` означает `Выключен`, код `1` означает `Ожидание`. |
| `Value` | `Read` | Любой поддержанный тип | Дополнительное значение для отображения. |
| `Visible` | `Read` | `Bool` | Показывает или скрывает карточку. |
| `Fault` | `Read` | `Bool` | Помечает карточку аварийной. |
| `StartCommand` | `ReadWrite` | `Bool` | Адрес команды `ПУСК`. |
| `StopCommand` | `ReadWrite` | `Bool` | Адрес команды `СТОП`. |
| `StartOffFeedback` | `Read` | `Bool` | При `true` отключает кнопку `ПУСК` и принудительно показывает `IsChecked=false`; команду в PLC не пишет. |
| `StopOffFeedback` | `Read` | `Bool` | При `true` отключает кнопку `СТОП` и принудительно показывает `IsChecked=false`; команду в PLC не пишет. |

Коды `Text` стандартной карточки:

| Код PLC | Текст | Цвет |
| --- | --- | --- |
| `0` | `Выключен` | `MutedTextBrush`, серый, legacy-синоним |
| `1` | `Ожидание` | `WarningBrush`, желтый |
| `2` | `Авария` | `FaultBrush`, красный |
| `3` | `Выполнение` | `ReadyBrush`, зеленый |
| `4` | `Выгрузка` | `ReadyBrush`, зеленый |
| `5` | `Загрузка` | `ReadyBrush`, зеленый |

`ПУСК` и `СТОП` всегда являются взаимоисключающими `ToggleButton`: включение `ПУСК` локально снимает `СТОП`, пишет `StopCommand=false`, затем `StartCommand=true`; включение `СТОП` локально снимает `ПУСК`, пишет `StartCommand=false`, затем `StopCommand=true`. Ручное снятие кнопки пишет только ее команду `false`. Если readback неожиданно вернул оба command-бита `true`, UI показывает только `СТОП` как приоритетный безопасный вариант и ничего молча не пишет обратно в PLC. Роли `StartOffFeedback` и `StopOffFeedback` существуют только для карточек: `true` отключает соответствующую кнопку и сбрасывает `IsChecked=false`, а `false` или отсутствие сигнала возвращает обычную доступность.

## Заглушки

`PlaceholderRules` не имеют Signal Binding ролей. Они управляют только layout-логикой автозаполнения карточек: где показывать пустые места, какой высоты они должны быть и как стилизуются.

## TopBar

| Роль | Направление | Тип | Назначение |
| --- | --- | --- | --- |
| `AutomaticModeCommand` | `ReadWrite` | `Bool` | Команда автоматического режима. Включение автомата пишет `AutomaticModeCommand=true` и `ManualModeCommand=false`. |
| `ManualModeCommand` | `ReadWrite` | `Bool` | Команда ручного режима. Включение ручного режима пишет `ManualModeCommand=true` и `AutomaticModeCommand=false`. |
| `EmergencyCommand` | `ReadWrite` | `Bool` | Команда аварии. Toggle `АВАРИЯ` пишет `true` при включении и `false` при снятии галочки. |

`AutomaticModeCommand`, `ManualModeCommand` и `EmergencyCommand` не используют OffFeedback-роли. `АВТОМАТ` и `РУЧНОЙ` остаются взаимоисключающими toggle-командами.

## Системные SignalId

| SignalId | Направление | Тип | Назначение |
| --- | --- | --- | --- |
| `connection.status` | `Read` | `String` | Статус общего Modbus Demo runtime, который отображается в TopBar. |
| `connection.connected` | `Read` | `Bool` | `true`, когда Modbus runtime running и snapshot не stale. |
| `system.fault` | `Read` | `Bool` | Общий сигнал аварии из `Modbus.DataMap`: при `true` все узлы, линии и карточки переходят в fault-вид, команды карточек блокируются; offline остается выше по приоритету. |

## Правило для ToggleButton

1. Узлы `Отправить`/`Возврат` пишут `true` при выборе роли и `false` при снятии или переключении на другой узел.
2. Режимы `АВТОМАТ`/`РУЧНОЕ` работают взаимоисключающе: включение одного режима пишет `true` в выбранный command и `false` в противоположный.
3. Карточные `ПУСК`/`СТОП` являются взаимоисключающими toggle-командами: включение одной сначала пишет `false` в противоположную команду, затем `true` в свою.
4. TopBar `АВАРИЯ` является обычной toggle-командой и пишет `true/false` напрямую в `EmergencyCommand`.
5. Snapshot/readback, включая `StartOffFeedback`/`StopOffFeedback`, только меняет UI-состояние и не инициирует записи команд.
