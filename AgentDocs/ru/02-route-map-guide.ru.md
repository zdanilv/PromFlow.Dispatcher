...
перед отправкой проверяет Modbus mapping для текущего `SignalId`, отправляет
`Write`/`ReadWrite` строки через обычный `SignalWriteRequest`, но не закрывает диалог.
