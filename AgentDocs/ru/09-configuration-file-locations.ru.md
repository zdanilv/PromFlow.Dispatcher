...
Содержит полное пользовательское определение RouteMap: геометрию, карточки, стили, bindings `SignalId`, параметры оборудования и настройки TopBar. Физические Modbus-адреса находятся не здесь, а в разделе `Modbus.DataMap` файла `appsettings.json`.
