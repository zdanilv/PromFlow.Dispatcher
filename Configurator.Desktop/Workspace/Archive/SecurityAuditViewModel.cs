...
public sealed class SecurityAuditViewModel : ViewModelBase
{
    private readonly IArchiveQueryService _queryService;

    public SecurityAuditViewModel(IArchiveQueryService queryService)
    {
        _queryService = queryService ?? throw new ArgumentNullException(nameof(queryService));
    }

    public ObservableCollection<ArchiveSecurityAuditRowViewModel> Records { get; } = [];

    public async Task<(bool Succeeded, long TotalCount, bool HasMore, string? Error)> LoadAsync(
        ArchiveQuery query,
        CancellationToken cancellationToken)
    {
        var outcome = await _queryService.QuerySecurityAuditAsync(query, cancellationToken).ConfigureAwait(true);
        ...
        {
            Records.Add(new ArchiveSecurityAuditRowViewModel(record));
        }
